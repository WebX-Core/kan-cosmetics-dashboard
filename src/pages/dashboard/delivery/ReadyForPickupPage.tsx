...
import { useBulkPickupNotification, useReadyForPickupOrders } from "@/features/commerce";
...
const shipmentLabel = (row: Row): string => {
  ...
  return text(
    ...
    text(shipment.trackingNumber, text(shipment.externalOrderId, text(shipment.status))),
  );
};
...
const columns = [
  {
    ...
    render: (row: Row) => (
      <div className="min-w-[150px]">
        <p className="text-[13px] font-semibold text-[#1d1d1f]">{text(row.orderNumber)}</p>
        <p className="mt-[3px] text-[12px] text-[#86868b]">{dateTime(row.createdAt)}</p>
      </div>
    ),
  },
  {
    ...
    render: (row: Row) => (
      <div className="min-w-[180px]">
        <p className="text-[13px] font-semibold text-[#1d1d1f]">{customerName(row)}</p>
        <p className="mt-[3px] text-[12px] text-[#86868b]">{customerPhone(row)}</p>
      </div>
    ),
  },
  {
    ...
    label: "Destination",
    render: (row: Row) => (
      <p className="max-w-[360px] text-[12px] leading-[18px] text-[#52525b]">
        {addressSummary(row)}
      </p>
    ),
  },
  {
    ...
    label: "Shipment",
    render: (row: Row) => (
      <span className="inline-flex rounded-full bg-emerald-50 px-[10px] py-[4px] text-[12px] font-semibold text-emerald-700 ring-1 ring-emerald-100">
        {shipmentLabel(row)}
      </span>
    ),
  },
  {
    ...
    render: (row: Row) => (
      <div className="text-[13px] font-semibold text-[#1d1d1f]">
        ...
        <p className="mt-[3px] text-[12px] font-normal text-[#86868b]">Shipping {money(row.shippingAmount)}</p>
      </div>
    ),
  },
];
...
export const ReadyForPickupPage: React.FC = () => {
  ...
  const { state, setState, debouncedSearch } = useListQueryState({ page: 1, limit: 50, search: "" });
  ...
  const payload = React.useMemo(() => listFromPayload(query.data), [query.data]);
  const [selectedIds, setSelectedIds] = React.useState<Set<string>>(new Set());

  React.useEffect(() => {
    const visible = new Set(payload.orders.map((row) => text(row.id, "")).filter(Boolean));
    setSelectedIds((previous) => new Set(Array.from(previous).filter((id) => visible.has(id))));
  }, [payload.orders]);

  const requestPickup = async (ids: Set<string>, clearSelection: () => void) => {
  ...
  return (
    <PageLayout
      ...
      actions={
        <button
          ...
          className="inline-flex h-[34px] items-center gap-[8px] rounded-full border border-[#d2d2d7] bg-white px-[16px] text-[13px] font-medium text-[#1d1d1f] transition hover:bg-[#f5f5f7]"
        >
          <RefreshCw size={13} />
          Refresh
        </button>
    ...
    >
      <div className="grid gap-[13px] md:grid-cols-3">
        ...
        <div className="rounded-xl border border-blue-100 bg-blue-50/70 p-[18px]">
          <div className="flex items-center gap-[8px] text-[13px] font-semibold text-blue-700">
            <Truck size={16} />
            Selected for pickup
          </div>
          <p className="mt-[12px] text-[28px] font-semibold tracking-[-0.03em] text-[#1d1d1f]">{selectedIds.size}</p>
        </div>
        <div className="rounded-xl border border-[#e5e5e7] bg-white p-[18px]">
          <p className="text-[13px] font-semibold text-[#1d1d1f]">How it works</p>
          <p className="mt-[8px] text-[12px] leading-[18px] text-[#6e6e73]">
            Select packed orders, then send one pickup notification. Pick & Drop handles rider pickup after they accept the request.
          </p>
        </div>
      </div>
      ...
      <DataTableV2
        ...
        rowActions={(row) => (
          <button
            ...
            className="rounded-full border border-[#d2d2d7] px-[12px] py-[5px] text-[12px] font-medium text-[#1d1d1f] transition hover:bg-[#f5f5f7]"
          >
            View
          </button>
        )}
        bulkActions={(ids, clearSelection) => (
          <button
            ...
            className="inline-flex h-[34px] items-center gap-[8px] rounded-full bg-[var(--primary)] px-[16px] text-[13px] font-semibold text-white transition hover:bg-[var(--primary-hover)] disabled:cursor-not-allowed disabled:opacity-60"
          >
            {bulkPickup.isPending ? <Loader2 size={14} className="animate-spin" /> : <Truck size={14} />}
