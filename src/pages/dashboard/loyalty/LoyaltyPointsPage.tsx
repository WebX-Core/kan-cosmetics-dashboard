...
export const LoyaltyPointsPage: React.FC = () => {
  ...
  const columns = [
    { key: "customer", label: "Customer", sortValue: (row: typeof rows[number]) => customerName(row.customer), render: (row: typeof rows[number]) => <div><p className="font-medium">{customerName(row.customer)}</p><p className="text-xs text-gray-500">{text(row.customer?.email, row.customerId)}</p></div> },
    { key: "points", label: "Status Points", sortValue: (row: typeof rows[number]) => number(row.points), render: (row: typeof rows[number]) => <span className={`font-semibold ${number(row.points) < 0 ? "text-red-600" : "text-emerald-700"}`}>{number(row.points) > 0 ? "+" : ""}{number(row.points).toLocaleString()}</span> },
    { key: "rewardPoints", label: "Wallet Points", sortValue: (row: typeof rows[number]) => number(row.rewardPoints), render: (row: typeof rows[number]) => <span className={`font-semibold ${number(row.rewardPoints) < 0 ? "text-red-600" : "text-emerald-700"}`}>{number(row.rewardPoints) > 0 ? "+" : ""}{number(row.rewardPoints).toLocaleString()}</span> },
    { key: "sourceType", label: "Source", sortValue: (row: typeof rows[number]) => row.sourceType, render: (row: typeof rows[number]) => <span className="text-xs font-medium">{row.sourceType.replaceAll("_", " ")}</span> },
    ...
    { key: "createdAt", label: "Created", sortValue: (row: typeof rows[number]) => text(row.createdAt), render: (row: typeof rows[number]) => date(row.createdAt) },
  ];
