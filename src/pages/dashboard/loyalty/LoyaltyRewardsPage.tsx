...
export const LoyaltyRewardsPage: React.FC = () => {
  ...
  const columns = [
    { key: "customer", label: "Customer", sortValue: (row: typeof rows[number]) => customerName(row.customer), render: (row: typeof rows[number]) => <div><p className="font-medium text-[#1d1d1f]">{customerName(row.customer)}</p><p className="text-xs text-[#86868b]">{text(row.customer?.email, row.customerId)}</p></div> },
    { key: "reward", label: "Reward", sortValue: (row: typeof rows[number]) => row.title, render: (row: typeof rows[number]) => <div><p className="font-medium">{row.title}</p><p className="text-xs text-[#86868b]">{row.rewardType} · {text(row.tierCode)}</p></div> },
    { key: "status", label: "Status", sortValue: (row: typeof rows[number]) => row.rewardStatus, render: (row: typeof rows[number]) => <StatusBadge status={row.rewardStatus} /> },
    { key: "expires", label: "Expires", sortValue: (row: typeof rows[number]) => text(row.expiresAt), render: (row: typeof rows[number]) => date(row.expiresAt) },
