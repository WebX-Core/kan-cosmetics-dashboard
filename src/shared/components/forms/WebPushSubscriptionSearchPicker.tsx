...
type Props = Readonly<{
  label: string;
  placeholder?: string;
  value: WebPushSubscriptionSearchOption | null;
  onChange: (subscription: WebPushSubscriptionSearchOption | null) => void;
  required?: boolean;
  helperText?: string;
}>;

const statusText = (value: boolean): string => (value ? "Active" : "Inactive");

export const WebPushSubscriptionSearchPicker: React.FC<Props> = ({
  label,
  placeholder = "Search subscriptions by endpoint, browser, or owner",
  value,
  onChange,
  required = false,
  helperText,
}) => {
  const { subscriptions, isSearching, search, setSearch } = useWebPushSubscriptionSearch();
  ...
  const handleSelect = (subscription: WebPushSubscriptionSearchOption) => {
  ...
  const selectedMeta = [value?.owner, value?.browser, value?.platform].filter(Boolean).join(" • ");
  const optionMeta = (subscription: WebPushSubscriptionSearchOption): string =>
    [subscription.owner, subscription.browser, subscription.platform].filter(Boolean).join(" • ");
      ...
      {value ? (
        <div className="flex items-start justify-between gap-3 rounded-xl border border-[#d2d2d7] bg-[#f5f5f7] px-4 py-3">
          <div className="min-w-0">
            <div className="truncate text-[14px] font-medium text-[#1d1d1f]">{value.endpoint || value.id}</div>
            {selectedMeta ? (
              <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[12px] text-[#6e6e73]">
                <span>{selectedMeta}</span>
                <span>•</span>
                <span className={value.isActive ? "text-emerald-600" : "text-amber-700"}>{statusText(value.isActive)}</span>
              </div>
            ) : (
          ...
          <button
            ...
            aria-label="Clear selected subscription"
          >
            <X size={12} />
          </button>
        </div>
              ...
              <div className="max-h-72 overflow-auto p-2">
                {subscriptions.length > 0 ? (
                  subscriptions.map((subscription) => (
                    ...
                    >
                      <div className="min-w-0">
                        <div className="truncate text-[14px] font-medium text-[#1d1d1f]">{subscription.endpoint || subscription.id}</div>
                        <div className="mt-1 text-[12px] text-[#6e6e73]">{optionMeta(subscription)}</div>
                      </div>
                      <div className="mt-0.5 flex items-center gap-2">
                        <span
                          className={`rounded-full px-2 py-1 text-[11px] font-medium ${
                            subscription.isActive ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"
                          }`}
                        >
                          {statusText(subscription.isActive)}
                        </span>
