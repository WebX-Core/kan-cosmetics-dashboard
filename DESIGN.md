...
The dashboard should use mostly neutral backgrounds and surfaces. Brand color should be reserved for primary actions, active states, selected filters, and key highlights. Status colors should only represent actual semantic states such as success, warning, danger, or info.

Main text must be clearly readable. Muted text should be softer but never too faint. Borders should be subtle but visible. Cards, tables, badges, buttons, filters, inputs, chart legends, and focus states must have clear contrast.

Do not make every dashboard card colorful. Do not use random gradients. Do not rely on color alone to communicate meaning. Use labels, icons, arrows, or text alongside status colors.

The final dashboard should feel premium, calm, readable, spacious, and trustworthy.
